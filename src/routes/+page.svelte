<script lang="ts">
	...
	const unwrappedDefaultQueries = derived(defaultCharacterQueries, ($queries) =>
		$queries.map((query) => query)
	);

	$: defaultCharacterResponses = $unwrappedDefaultQueries
		.map((query) => {
	...
	const searchQuery = createQuery({
		...
		queryFn: async () =>
			(await fetch(`https://api.disneyapi.dev/character?name=${$nameToSearch}`)).json()
	});

	const showSearchResults = derived(
		[nameToSearch, searchQuery],
		([$nameToSearch, $searchQuery]) => {
